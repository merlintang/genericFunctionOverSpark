...
public class RTree<T> implements java.io.Serializable
{
  ...
  public int getPartitionID(float[] coords)
  {
    ...
    if(nodes.size()>0)
    {
      //return the first touched leaf id
      return (int)nodes.get(0).id;
    }

     return  0;
  }

  private List<Node> searchLeavesForPt(float[] coords)
  {
    assert (coords.length == numDims);
  ...
  /**
   * get the iterator for entry
   * @return
   */
  public ArrayList<Entry> iterators()
  {
    ArrayList<Entry> results =new  ArrayList<Entry>();
    this.getIterator(this.root,results);
    return results;
  }

  private void getIterator(Node n, ArrayList<Entry> results)
  {
    if (n.leaf)
    {
